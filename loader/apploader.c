...
#define DBGPRINT(textp) \
  DBG(chSequentialStreamWrite(DBGCH, (const uint8_t *)(textp), \
                              strlen((const char *)(textp))))
...
static ldr_err_t expect_char(BaseChannel *chp, const char c) {
  ...
  if (chSequentialStreamGet(chp) != (msg_t)c) return LDR_ERR_BADVAL;
  return LDR_OK;
}
...
static ldr_err_t expect_char_ws(BaseChannel *chp, const char c) {
  ...
  register msg_t r;
  while ((r = chSequentialStreamGet(chp)) != (const msg_t)c) {
...
static ldr_err_t expect_nl(BaseChannel *chp) {
  ...
  if (chSequentialStreamGet(chp) != '\r') return LDR_ERR_BADVAL;
  if (chSequentialStreamGet(chp) != '\n') return LDR_ERR_BADVAL;
  return LDR_OK;
}

static ldr_err_t read_char(BaseChannel *chp, char *cp) {

  msg_t c = chSequentialStreamGet(chp);
...
static ldr_err_t write_char(BaseChannel *chp, const char c) {
  ...
  if (chSequentialStreamPut(chp, c) != Q_OK) return LDR_ERR_EOF;
  return LDR_OK;
}

static ldr_err_t write_nl(BaseChannel *chp) {

  if (chSequentialStreamPut(chp, '\r') != Q_OK) return LDR_ERR_EOF;
  if (chSequentialStreamPut(chp, '\n') != Q_OK) return LDR_ERR_EOF;
  return LDR_OK;
}
...
static ldr_err_t write_byte(BaseChannel *chp, uint8_t byte) {
  ...
  if (chSequentialStreamPut(chp, hex2char[byte >> 4]) != Q_OK)
    return LDR_ERR_EOF;
  if (chSequentialStreamPut(chp, hex2char[byte & 0x0F]) != Q_OK)
    return LDR_ERR_EOF;
  return LDR_OK;
}
...
static ldr_err_t write_dword(BaseChannel *chp, uint32_t ptr) {
  ...
  for (i = 2 * sizeof(uint32_t); i--; ptr <<= 4) {
    if (Q_OK != chSequentialStreamPut(chp,
         hex2char[(ptr >> (sizeof(uint32_t) * 8 - 4)) & 0x0F]))
      return LDR_ERR_EOF;
  }
  return LDR_OK;
}
...
#define CHECK(x) { register ldr_err_t e = (x); if (e != LDR_OK) return e; }
  ...
  for (;;) {
    chSequentialStreamWrite(cfgp->chp, (const uint8_t *)",\r\n", 3);
